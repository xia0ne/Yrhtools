...
script-update() {
cd $workfile;
wget -N --no-check-certificate https://raw.githubusercontent.com/xia0ne/Yrhtools/master/tools.sh
}
########################################################################################################开始执行命令
#############颜色变量
startblue="\033[36m"
...
if [ $? == "1" ]; then {
   ...
   {
echo -e "脚本文件夹位于/home/tools"; cd /root/tools
   }
fi
####################################
#进入主界面
